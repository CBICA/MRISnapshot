...
def overlayImage(img2d_under, img2d_over, transparency=0.6, is_edge=0):
    ...
    tmpData =  np.uint8(np.tile(np.expand_dims(img2d_over,3),(1,1,4))*255)
...
def overlayImageDouble(img2d_under, img2d_over1, img2d_over2, transparency=0.6, is_edge=0):
    ...
    tmpData1 =  np.uint8(np.tile(np.expand_dims(img2d_over1,3),(1,1,4))*255)
    tmpData1[:,:,[1,2]] = 0        # Set RED color

    tmpData2 =  np.uint8(np.tile(np.expand_dims(img2d_over2,3),(1,1,4))*255)
    tmpData2[:,:,[0,1]] = 0        # Set BLUE color
